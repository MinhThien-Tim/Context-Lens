...
const documentRecord: DocumentRecord = { id: 'backup-doc', title: 'Reading', content: 'Text.', safeHtml: '<h2>Reading</h2><p><strong>Text.</strong></p>', kind: 'text', createdAt: 1, updatedAt: 2, location: { kind: 'text', scrollY: 0, progress: 0, updatedAt: 2 }, data: new Blob(['private binary']) };
...
describe('portable backup', () => {
  ...
  it('validates and restores a versioned backup', async () => {
    ...
    expect((await db.documents.get('backup-doc'))?.safeHtml).toBe('<h2>Reading</h2><p><strong>Text.</strong></p>');
