...
export function PdfReadingPage({ page }: { page: PdfStructuredPage }) {
  return <section class="pdf-reading-page" data-pdf-reading-page={page.pageNumber} data-reader-text aria-labelledby={`pdf-page-label-${page.pageNumber}`}>
    ...
    <div class="pdf-reading-content">{page.blocks.length ? page.blocks.map(block => <PdfReadingBlock key={block.id} block={block} />) : <p class="pdf-reading-empty">This page has no extractable text. Switch to Original to view the page.</p>}</div>
  </section>;
}
