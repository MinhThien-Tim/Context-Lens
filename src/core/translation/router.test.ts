...
describe('TranslationRouter', () => {
  ...
  it('uses Google after uncertain MyMemory even when provider priorities are reversed', async () => {
    const google = provider('online-auto', 1);
    const mymemory = provider('mymemory', 2, vi.fn().mockResolvedValue({ ...result, text: 'phí' }));
    const routed = await new TranslationRouter([google, mymemory], cache()).translate({ ...input, text: 'charge', localContext: { meaningsVi: ['buộc tội', 'sạc'], senseConfidence: 0.3 } });
    expect(routed.provider).toBe('online-auto');
    expect(google.translate).toHaveBeenCalledTimes(1);
  });
