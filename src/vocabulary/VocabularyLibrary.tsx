...
export function VocabularyLibrary({ records, onClose, onDelete }: { records: VocabularyRecord[]; onClose: () => void; onDelete: (id: string) => void }) {
  ...
  return <div class="modal-layer">
    ...
    <section ref={dialogRef} tabIndex={-1} class="settings-modal vocabulary-modal" role="dialog" aria-modal="true" aria-labelledby="vocabulary-title">
      ...
      <div class="export-actions"><button class="secondary-button" disabled={!records.length} onClick={() => download('context-lens-vocabulary.csv', vocabularyCsv(records), 'text/csv;charset=utf-8')}>Export CSV</button><button class="primary-button" disabled={!records.length} onClick={() => download('context-lens-english101.json', JSON.stringify(buildEnglish101Export(records), null, 2), 'application/json')}>English101 JSON</button></div>
    </section>
  </div>;
}
