...
export function ReaderSettings({ value, onChange, onClose }: { value: AppPreferences; onChange: (value: AppPreferences) => void; onClose: () => void }) {
  ...
  return (
    <section class="popover settings-popover" aria-label="Reader settings">
      <div class="popover-title"><strong>Reading</strong><button class="icon-button" onClick={onClose} aria-label="Close reader settings">×</button></div>
      ...
      <div class="segmented">
        <button class={value.fontFamily === 'serif' ? 'active' : ''} onClick={() => set('fontFamily', 'serif')}>Serif</button>
        <button class={value.fontFamily === 'sans' ? 'active' : ''} onClick={() => set('fontFamily', 'sans')}>Sans</button>
      </div>
      <div class="segmented">
        {(['light', 'system', 'dark'] as const).map((theme) => <button class={value.theme === theme ? 'active' : ''} onClick={() => set('theme', theme)}>{theme}</button>)}
      </div>
    </section>
  );
}
