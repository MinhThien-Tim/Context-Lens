...
export function App() {
  ...
  const [activeHighlightColor, setActiveHighlightColor] = useState<'yellow' | 'pink' | 'blue' | null>(null);
  ...
  return (
    <ReaderShell contentsOpen={contentsOpen} contextOpen={lookupOpen || showNotes}>
      ...
      <ReaderToolbar title={documentRecord.title} contentsOpen={contentsOpen} highlightAvailable={documentRecord.kind === 'pdf' && pdfMode === 'reading'} highlightActive={activeHighlightColor !== null} highlightOpen={highlightToolsOpen} onHighlight={() => setHighlightToolsOpen(value => !value)} onBack={() => void closeDocument()} onContents={() => setContentsOpen(!contentsOpen)} onNote={() => openNotes(null)} onSettings={() => setShowReaderSettings(!showReaderSettings)} onEngines={() => setShowApiSettings(true)}>
      ...
      {highlightToolsOpen && documentRecord.kind === 'pdf' && pdfMode === 'reading' && <div class="reader-highlight-palette" role="dialog" aria-label="Highlight settings"><button class={`highlight-tool-toggle ${activeHighlightColor ? 'active' : ''}`} aria-pressed={activeHighlightColor !== null} onClick={() => { setActiveHighlightColor(value => value ? null : 'yellow'); setHighlightToolsOpen(false); }}><svg aria-hidden="true" width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="m14.5 4.5 5 5L9 20H4v-5Z"/><path d="m12 7 5 5M4 22h7"/></svg><span>Highlight</span><small>{activeHighlightColor ? 'On' : 'Off'}</small></button><div class="reader-highlight-colors" role="group" aria-label="Highlight color">{(['yellow', 'pink', 'blue'] as const).map(color => <button key={color} class={`highlight-color highlight-color-${color} ${activeHighlightColor === color ? 'selected' : ''}`} aria-label={`Use ${color} highlight`} aria-pressed={activeHighlightColor === color} onClick={() => { setActiveHighlightColor(color); setHighlightToolsOpen(false); }} />)}</div></div>}
      ...
      {documentRecord.kind === 'pdf' && pdfMode === 'original' && currentLocation.kind === 'pdf'
        ...
        ? <PdfReadingView documentRecord={documentRecord} location={currentLocation} style={readerStyle} activeHighlightColor={activeHighlightColor} onOriginal={() => changePdfViewMode('original')} onLocation={location => { setCurrentLocation(location); setProgress(location.progress); void db.documents.update(documentRecord.id, { location, updatedAt: Date.now() }); }} onLookup={runLookup} onAddNote={selection => openNotes(selection)} onHighlight={highlight => { const current = documentRecord.highlights ?? []; const match = current.find(item => item.startOffset === highlight.startOffset && item.endOffset === highlight.endOffset); const highlights = match ? current.map(item => item.id === match.id ? { ...item, color: highlight.color } : item) : [...current, highlight]; setDocumentRecord({ ...documentRecord, highlights }); void db.documents.update(documentRecord.id, { highlights, updatedAt: Date.now() }); }} />
